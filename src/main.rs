...
fn main() -> Result<(), Box<dyn std::error::Error>> {
  ...
  // Confirm the program launched
  println!("hello, world");

  // Set up a callback for mouse events
  mouse_manager.hook(Box::new(|event| {

    // Check if this event was a left click
    if let MouseEvent::Press(mouce::common::MouseButton::Left) = event {
      ...
      // Convert title to rust string and print
      ...
      // Set whether window is layered, transparent
      unsafe { SetWindowLongPtrW(window_handle, GWL_EXSTYLE, (new_style as LONG).try_into().unwrap()) };

      // Opaqueness parameters
      let opaque_value = 80; // Adjust this value as needed (0 - fully transparent, 255 - fully opaque)
      let key_color = COLORREF::default(); // Specify a key color if needed

      // Set window attributes for opaqueness
      unsafe { SetLayeredWindowAttributes(window_handle, key_color, opaque_value, LWA_ALPHA | LWA_COLORKEY) };
    }
  }))?;
  loop {
